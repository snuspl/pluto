...
final class DefaultQueryPartitionerImpl implements QueryPartitioner {
   ...
   */
  @Override
  public PhysicalPlan<PartitionedQuery, StreamType.Direction> chainOperators(
          final PhysicalPlan<Operator, StreamType.Direction> plan) {
    // This is a map of source and PartitionedQueries which are following sources
    final Map<Source, Map<PartitionedQuery, StreamType.Direction>> sourceMap = new HashMap<>();
    ...
    final DAG<PartitionedQuery, StreamType.Direction> partitionedQueryDAG = new AdjacentListDAG<>();
    ...
    for (final Map.Entry<Source, Map<Operator, StreamType.Direction>> entry : plan.getSourceMap().entrySet()) {
      // This is the root operators which are directly connected to sources.
      final Map<Operator, StreamType.Direction> rootOperatorEdges = entry.getValue();
      final Map<PartitionedQuery, StreamType.Direction> partitionedQueries = new HashMap<>();
      for (final Map.Entry<Operator, StreamType.Direction> opAndEdge : rootOperatorEdges.entrySet()) {
        final Operator rootOperator = opAndEdge.getKey();
        final StreamType.Direction edgeDirection = opAndEdge.getValue();
   ...
   */
  private void chainOperatorHelper(final PhysicalPlan<Operator, StreamType.Direction> plan,
                                   final Map<Operator, PartitionedQuery> partitionedQueryMap,
                                   final DAG<PartitionedQuery, StreamType.Direction> partitionedQueryDAG,
    ...
    final DAG<Operator, StreamType.Direction> dag = plan.getOperators();
    final Map<Operator, StreamType.Direction> nextEdges = dag.getEdges(currentOp);
    for (final Map.Entry<Operator, StreamType.Direction> opAndEdge : nextEdges.entrySet()) {
